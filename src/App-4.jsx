...
function SeatingApp({ user, event, onBack }) {
  ...
  // מסך תקופת ניסיון הסתיימה
  if(trialExpired){
    return(
      <div dir="rtl" style={{minHeight:"100vh",background:C.bg,fontFamily:"'Heebo',sans-serif",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{background:C.surface,borderRadius:24,padding:32,maxWidth:400,width:"100%",textAlign:"center",boxShadow:"0 8px 40px rgba(27,58,140,.12)"}}>
          <div style={{fontSize:56,marginBottom:16}}>⏰</div>
          <div style={{fontSize:22,fontWeight:900,color:C.text,marginBottom:8}}>תקופת הניסיון הסתיימה</div>
          <div style={{fontSize:14,color:C.muted,lineHeight:1.7,marginBottom:24}}>
            תקופת הניסיון החינמי של 24 שעות הסתיימה.<br/>
            שדרג לחבילה כדי להמשיך לנהל את האירוע שלך.
          </div>
          <div style={{background:C.blueXL,borderRadius:14,padding:"14px 16px",marginBottom:20,border:`1.5px solid ${C.border}`}}>
            <div style={{fontSize:13,fontWeight:700,color:C.blue,marginBottom:4}}>חבילת בסיס + הושבה</div>
            <div style={{fontSize:28,fontWeight:900,color:C.blue}}>₪150</div>
            <div style={{fontSize:12,color:C.muted}}>תשלום חד פעמי לאירוע</div>
          </div>
          <button onClick={()=>setScreen("packages")}
            style={{width:"100%",background:`linear-gradient(135deg,${C.blueM},${C.blueL})`,color:"#fff",border:"none",borderRadius:14,padding:"14px",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"inherit",marginBottom:10,boxShadow:`0 4px 16px ${C.blueL}44`}}>
            👑 צפה בחבילות ושדרג
          </button>
        ...
        {screen==="packages"&&<div style={{position:"fixed",inset:0,zIndex:300,background:C.bg,overflowY:"auto"}}><PackagesScreen event={event} onBack={()=>setScreen("")}/></div>}
      </div>
    );
  }
...
function AdminLogin({ onSuccess, onClose }) {
  ...
  const submit=async()=>{
    ...
    if(error){
      ...
      const{data:d2,error:e2}=await sb.auth.signUp({
        email:ADMIN_EMAIL,
        password:ADMIN_PASSWORD
      });
      if(e2){setErr("שגיאה: "+e2.message);setLoad(false);return;}
      if(d2.user)onSuccess(d2.user);
    } else {
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#0D1117",direction:"rtl",fontFamily:"'Heebo',sans-serif"}}>
      <div style={{background:"#161B22",border:"1px solid #30363D",borderRadius:20,padding:"40px 32px",width:"100%",maxWidth:360,textAlign:"center"}}>
        ...
        <input
          ...
          placeholder="סיסמה"
          autoFocus
          style={{width:"100%",background:"#21262D",border:`1.5px solid ${err?"#F85149":"#30363D"}`,borderRadius:12,padding:"13px 16px",fontSize:15,color:"#E6EDF3",outline:"none",fontFamily:"inherit",boxSizing:"border-box",marginBottom:err?8:16,textAlign:"right"}}
        />
        {err&&<div style={{fontSize:13,color:"#F85149",marginBottom:16,textAlign:"right"}}>{err}</div>}
        <button onClick={submit} disabled={load||!pass}
          style={{width:"100%",background:load||!pass?"#21262D":"#1B3A8C",color:"#fff",border:"none",borderRadius:12,padding:"13px",fontSize:15,fontWeight:700,cursor:load||!pass?"default":"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
          {load?<><Spinner size={18} color="#fff"/>מתחבר...</>:"כניסה לאדמין ←"}
        </button>
        <button onClick={onClose} style={{background:"none",border:"none",color:"#888",fontSize:13,cursor:"pointer",marginTop:16,fontFamily:"inherit"}}>
          חזרה לאתר
        </button>
      </div>
    </div>
