        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[
            ...
            <Card key={item.nav}
              ...
              style={{padding:"18px 16px",cursor:"pointer",display:"flex",flexDirection:"column",gap:8,border:`1.5px solid ${item.highlight?C.blueL:item.highlight2?"#7C3AED":C.border}`,background:item.highlight?C.blueXL:item.highlight2?"#F5F3FF":C.surface}}>
              <div style={{width:44,height:44,borderRadius:13,background:item.highlight?`linear-gradient(135deg,${C.blueM},${C.blueL})`:item.highlight2?"linear-gradient(135deg,#7C3AED,#A855F7)":C.blueXL,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{item.icon}</div>
              <div style={{fontWeight:700,fontSize:14,color:item.highlight?C.blue:item.highlight2?"#7C3AED":C.text}}>{item.label}</div>
            </Card>
          ))}
        </div>
      </div>
    ...
    return(<div dir="rtl" style={{fontFamily:"'Heebo',sans-serif",background:C.bg,minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${C.blue},${C.blueM})`,padding:"12px 16px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        ...
        <span style={{flex:1,fontWeight:800,fontSize:15,color:"#fff",overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>
          {screen==="home"?event.name:screen==="seating"?"🪑 סידורי הושבה":screen==="add"?"➕ הוסף אורח":screen==="import"?"📊 ייבוא אורחים":screen==="rsvp"?"✅ אישורי הגעה":screen==="invite"?"💌 הזמנה דיגיטלית":screen==="budget"?"💰 ניהול תקציב":screen==="ai"?"🤖 AI סידור חכם":screen==="settings"?"⚙️ הגדרות":event.name}
        </span>
