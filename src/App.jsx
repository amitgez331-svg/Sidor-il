      ...
      <section style={{padding:"88px 6vw",background:C.bg,textAlign:"center"}} id="pricing">
        <div style={{maxWidth:1080,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:20,maxWidth:1040,margin:"0 auto 40px"}}>
            {[
              {
                id:"basic",name:"בסיס + הושבה",price:"₪150",period:"לאירוע",color:C.blue,bg:C.blueXL,
                badge:null,
                features:["שליחה ידנית בוואטסאפ","קבלת אישורי הגעה","סידורי הושבה מלאים","מפה אינטראקטיבית","פתק הושבה להדפסה"],
              },
              {
                id:"sms",name:"הודעות SMS",price:"₪149",period:"+ חבילת SMS",color:"#059669",bg:"#F0FFF6",
                badge:"📱 SMS",
                features:["הכל בחבילת בסיס","שליחת SMS לכל האורחים","תזמון הודעות אוטומטי","אישורי הגעה ב-SMS","תזכורות לפני האירוע"],
              },
              {
                id:"whatsapp",name:"הודעות WhatsApp",price:"₪249",period:"עד 100 מוזמנים",color:"#25D366",bg:"#F0FFF6",
              ...
              {
                id:"vip",name:"VIP הכל כלול",price:"₪449",period:"לאירוע",color:"#B45309",bg:"#FFFBEB",
                badge:"👑 VIP",
                features:["הכל בחבילת WhatsApp","מספר שולחן ב-SMS ביום האירוע","תזכורות מרובות","סגנון הזמנה פרימיום","תמיכה ייעודית 24/7"],
              },
            ].map((p,i)=>(
          ...
          <div className="fu" style={{opacity:0,transform:"translateY(22px)",transition:"opacity .6s .2s,transform .6s .2s",background:C.surface,borderRadius:20,padding:"32px",border:`1px solid ${C.border}`,maxWidth:700,margin:"0 auto"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:12}}>
              {[[150,"₪69"],[300,"₪89"],[500,"₪119"],[800,"₪169"],[1000,"₪199"],[2000,"₪329"]].map(([sms,price])=>(
...
function AuthDrawer({ mode:initMode, onClose, onAuth }) {
  const [mode,setMode]=useState(initMode),[email,setEmail]=useState(""),[pass,setPass]=useState(""),[err,setErr]=useState(""),[load,setLoad]=useState(false),[googleLoad,setGoogleLoad]=useState(false);
      ...
      <div style={{padding:"20px 16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          {[
            ...
            {icon:"📱",label:"הודעות SMS",nav:"sms",locked:true},
            {icon:"💬",label:"הודעות WhatsApp",nav:"whatsapp",locked:true},
...
function PackagesScreen({ event, onBack }) {
  ...
  const packages=[
    {id:"basic",name:"בסיס + הושבה",price:150,color:C.blue,icon:"🪑",
    ...
    {id:"sms",name:"הודעות SMS",price:149,color:"#059669",icon:"📱",badge:"SMS",
     features:["הכל בחבילת בסיס","שליחת SMS לכל האורחים","תזמון הודעות אוטומטי","תזכורות לפני האירוע","אישורי הגעה ב-SMS"]},
    {id:"whatsapp",name:"WhatsApp אוטומטי",price:249,color:"#25D366",icon:"💬",badge:"WhatsApp",featured:true,
     features:["הכל בחבילת SMS","שליחה אוטומטית בוואטסאפ","תזכורת לפני האירוע","הודעת תודה לאחר האירוע","מגיב אוטומטי לשאלות"]},
    {id:"vip",name:"VIP הכל כלול",price:449,color:"#B45309",icon:"👑",badge:"VIP",
     features:["הכל בחבילת WhatsApp","מספר שולחן ב-SMS ביום האירוע","סגנון הזמנה פרימיום","תמיכה ייעודית 24/7","עדיפות בתמיכה"]},
  ];

  const smsPacks=[[150,69],[300,89],[500,119],[800,169],[1000,199],[2000,329]];
          ...
          <div key={pkg.id} onClick={()=>setSelectedPkg(selectedPkg?.id===pkg.id?null:pkg)}
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:selectedPkg?.id===pkg.id?12:0}}>
              ...
              <div style={{textAlign:"left"}}>
                ...
                <div style={{fontSize:10,color:C.muted}}>לאירוע</div>
              </div>
            </div>
            {selectedPkg?.id===pkg.id&&(
              <div>
                ...
                {/* אם SMS — הצג אפשרות לרכוש SMS */}
                {(pkg.id==="sms"||pkg.id==="whatsapp"||pkg.id==="vip")&&(
                  <div style={{background:C.bg,borderRadius:12,padding:12,marginBottom:12}}>
                    <div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:8}}>הוסף חבילת SMS (אופציונלי):</div>
                ...
                <button onClick={e=>{e.stopPropagation();handlePurchase(pkg,smsPkg);}}
                  ...
                  לרכישה מהירה ← {smsPkg?`₪${pkg.price+smsPkg[1]}`:`₪${pkg.price}`}
                </button>
              </div>
            )}
          </div>
...
function WhatsAppScreen({ event, guests }) {
  ...
  const schedules=[
    {id:"invite",label:"1. הזמנות",icon:"💌",channel:"WhatsApp",date:"לפי בחירה",desc:"שליחת הזמנה דיגיטלית עם לינק לאישור הגעה"},
    {id:"reminder",label:"2. סבב נוסף",icon:"🔔",channel:"WhatsApp",date:"שבוע לפני",desc:"תזכורת למי שלא אישר עדיין"},
    {id:"phone",label:"3. טלפונים",icon:"📞",channel:"מוקד אנושי",date:"3 ימים לפני",desc:"שיחות טלפון לאורחים שלא הגיבו"},
    {id:"reminder2",label:"4. תזכורות",icon:"⏰",channel:"SMS",date:"שבוע לפני האירוע",desc:"תזכורת SMS לכל האורחים"},
    {id:"table",label:"5. מס' שולחן",icon:"🪑",channel:"SMS",date:"ביום האירוע",desc:"שליחת מספר שולחן לכל אורח"},
    {id:"thanks",label:"6. תודות",icon:"🙏",channel:"SMS",date:"עד 7 ימים אחרי",desc:"הודעת תודה לאחר האירוע"},
  ];
  ...
  return(
    <div style={{direction:"rtl",fontFamily:"'Heebo',sans-serif",padding:16,paddingBottom:80}}>
      ...
      <div style={{background:"linear-gradient(135deg,#075E54,#25D366)",borderRadius:16,padding:"16px",marginBottom:16,color:"#fff"}}>
        ...
        <div style={{fontSize:12,opacity:.85}}>6 מועדים אוטומטיים לשליחת הודעות לאורחים</div>
      </div>

      {/* לוח זמנים */}
      <div style={{fontSize:13,fontWeight:800,color:C.text,marginBottom:12}}>המועדים שנקבעו אוטומטית:</div>
      {schedules.map((s,i)=>(
        <div key={s.id} onClick={()=>setMsgType(msgType===s.id?null:s.id)}
          style={{background:C.surface,border:`1.5px solid ${msgType===s.id?"#25D366":C.border}`,borderRadius:14,padding:"14px 16px",marginBottom:10,cursor:"pointer"}}>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{width:44,height:44,borderRadius:12,background:s.channel==="WhatsApp"?"#E8F5E9":s.channel==="SMS"?"#E3F2FD":"#FFF3E0",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0}}>{s.icon}</div>
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:2}}>
                <span style={{fontWeight:800,fontSize:14,color:C.text}}>{s.label}</span>
                <span style={{fontSize:10,fontWeight:700,background:s.channel==="WhatsApp"?"#25D366":s.channel==="SMS"?C.blue:"#F59E0B",color:"#fff",borderRadius:100,padding:"2px 8px"}}>{s.channel}</span>
              </div>
              <div style={{fontSize:11,color:C.muted}}>{s.date}</div>
            </div>
            <div style={{fontSize:12,color:msgType===s.id?"#25D366":C.muted}}>
              {msgType===s.id?"▲":"▼"}
            </div>
          </div>
          {msgType===s.id&&(
            <div style={{marginTop:12,paddingTop:12,borderTop:`1px solid ${C.border}`}}>
              <div style={{fontSize:13,color:C.muted,marginBottom:10}}>{s.desc}</div>
              <div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:6}}>תאריך שליחה:</div>
              <input type="datetime-local" value={scheduled} onChange={e=>setScheduled(e.target.value)}
                style={{width:"100%",background:C.bg,border:`1px solid ${C.border}`,borderRadius:10,padding:"9px 12px",fontSize:13,color:C.text,outline:"none",fontFamily:"inherit",boxSizing:"border-box",marginBottom:10}}/>
              <button onClick={e=>{e.stopPropagation();alert(`תזמון "${s.label}" נשמר!`);}}
                style={{width:"100%",background:"linear-gradient(135deg,#075E54,#25D366)",color:"#fff",border:"none",borderRadius:10,padding:"10px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                💬 תזמן שליחה
              </button>
            </div>
          )}
      ...
      <Card style={{padding:16,marginTop:8}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {[["עד 100 מוזמנים","₪249"],["עד 200 מוזמנים","₪349"],["עד 300 מוזמנים","₪449"],["עד 500 מוזמנים","₪549"],["עד 800 מוזמנים","₪749"],["ללא הגבלה","₪999"]].map(([label,price])=>(
            <div key={label} onClick={()=>alert(`בקרוב! תשלום ${price} דרך PayPlus`)}
              style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",background:C.bg,border:`1.5px solid ${C.border}`,borderRadius:12,cursor:"pointer"}}
              ...
              <span style={{fontSize:16,fontWeight:800,color:"#25D366"}}>{price} ←</span>
            </div>
          ))}
        </div>
      </Card>
