...
// ─── HAMBURGER MENU ───────────────────────────────────────────────────────────
function HamburgerMenu({ onOpenAuth, onClose, onVenuePage }) {
  const menuItems = [
    { label:"ראשי", href:"#" },
...
function LandingPage({ onOpenAuth }) {
  ...
  return(
    <div dir="rtl" style={{fontFamily:"'Heebo',sans-serif",background:C.bg,color:C.text,minHeight:"100vh"}}>
      ...
      {menuOpen&&<HamburgerMenu onOpenAuth={onOpenAuth} onClose={()=>setMenuOpen(false)} onVenuePage={()=>setVenuePage(true)}/>}
...
export default function Root() {
  ...
  const selectEvent=(ev)=>{
    setEvent(ev);
    if(ev)localStorage.setItem("lastEventId",ev.id);
  ...
  useEffect(()=>{
    ...
    const{data:{subscription}}=sb.auth.onAuthStateChange((_,session)=>{
      if(!session?.user){setUser(null);setEvent(null);localStorage.removeItem("lastEventId");}
      else setUser(session.user);
    });
    return()=>subscription.unsubscribe();
  },[]);

  const logout=async()=>{await sb.auth.signOut();setUser(null);selectEvent(null);setShowLanding(false);};
