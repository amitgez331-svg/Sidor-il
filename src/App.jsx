...
// ─── ADMIN DASHBOARD ─────────────────────────────────────────────────────────
const ADMIN_EMAIL="Sidoril2026@gmail.com";
...
function AdminDashboard({ user, onLogout }) {
  ...
  const loadData=async()=>{
    setLoading(true);
    const [{data:evData},{data:gData}]=await Promise.all([
      ...
      <div style={{padding:"24px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:24}}>
          {[[stats.users,"👥","משתמשים","#1B3A8C"],[stats.events,"📅","אירועים","#059669"],[stats.guests,"🎊","אורחים","#B45309"]].map(([v,ic,l,col])=>(
            <div key={l} style={{background:"#161B22",border:"1px solid #30363D",borderRadius:14,padding:"20px",borderTop:`3px solid ${col}`}}>
              <div style={{fontSize:32,marginBottom:4}}>{ic}</div>
              <div style={{fontSize:36,fontWeight:900,color:col}}>{v.toLocaleString()}</div>
        ...
        {/* חיפוש */}
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="🔍 חיפוש לפי שם אירוע או User ID..."
          style={{width:"100%",background:"#161B22",border:"1px solid #30363D",borderRadius:10,padding:"10px 16px",fontSize:14,color:"#E6EDF3",outline:"none",fontFamily:"inherit",boxSizing:"border-box",marginBottom:16}}/>

        {/* טאבים */}
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          {[["users","👥 משתמשים"],["events","📅 אירועים"]].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)}
              style={{background:tab===id?"#1B3A8C":"#21262D",color:"#E6EDF3",border:`1px solid ${tab===id?"#4A7AFF":"#30363D"}`,borderRadius:8,padding:"8px 20px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
              {label}
            </button>
          ))}
        </div>
        ...
        {loading?(
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {users.filter(u=>!search||u.events.some(e=>e.name?.toLowerCase().includes(search.toLowerCase()))).map(u=>(
              <div key={u.user_id} style={{background:"#161B22",border:"1px solid #30363D",borderRadius:12,padding:"16px 20px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                  <div>
                    <div style={{fontSize:12,color:"#888",marginBottom:4}}>User ID</div>
                    <div style={{fontSize:13,fontWeight:700,color:"#E6EDF3",fontFamily:"monospace"}}>{u.user_id}</div>
                  </div>
                  <div style={{display:"flex",gap:8}}>
                    <span style={{background:"#1B3A8C22",color:"#4A7AFF",border:"1px solid #4A7AFF44",borderRadius:100,padding:"3px 10px",fontSize:12,fontWeight:700}}>{u.events.length} אירועים</span>
                    <span style={{background:"#05906922",color:"#10B981",border:"1px solid #10B98144",borderRadius:100,padding:"3px 10px",fontSize:12,fontWeight:700}}>{u.guests} אורחים</span>
                  </div>
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {u.events.map(ev=>(
                    <span key={ev.id} style={{background:"#21262D",border:"1px solid #30363D",borderRadius:8,padding:"4px 10px",fontSize:12,color:"#E6EDF3"}}>
            ...
            {filteredEvents.map(ev=>(
              <div key={ev.id} style={{background:"#161B22",border:"1px solid #30363D",borderRadius:12,padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                <div>
                  ...
                  <div style={{fontSize:10,color:"#555",fontFamily:"monospace",marginTop:2}}>{ev.user_id}</div>
                </div>
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <span style={{background:ev.invite_active?"#05906922":"#DA363322",color:ev.invite_active?"#10B981":"#F85149",border:`1px solid ${ev.invite_active?"#10B98144":"#F8514944"}`,borderRadius:100,padding:"3px 10px",fontSize:12,fontWeight:700}}>
                    {ev.invite_active?"✓ הזמנה פעילה":"✗ הזמנה כבויה"}
                  </span>
                </div>
              </div>
            ))}
...
export default function Root() {
  ...
  if(isAdmin){
    ...
    if(!user||user.email!=="Sidoril2026@gmail.com")return(
