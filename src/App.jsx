          ...
          <div key={pkg.id} onClick={()=>setSelectedPkg(selectedPkg?.id===pkg.id?null:pkg)}
            ...
            {selectedPkg?.id===pkg.id&&(
              <div>
                ...
                {/* עמדת קבלת פנים — צור קשר בלבד */}
                {pkg.id==="reception"?(
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    <a href="https://wa.me/972526817102?text=שלום, אני מעוניין לשמוע על עמדת קבלת פנים" target="_blank" rel="noopener"
                  ...
                  <button onClick={e=>{e.stopPropagation();
                    ...
                    לרכישה מהירה ← {smsPkg?`₪${smsPkg[1].toLocaleString()}`:`₪${pkg.price.toLocaleString()}`}
                  </button>
                )}
              </div>
            )}
