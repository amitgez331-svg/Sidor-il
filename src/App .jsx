...
function MobileRsvpScreen({ guests, tables, event, sb, setGuests, setTables, setModal }) {
  ...
  return(
    <div style={{direction:"rtl",fontFamily:"'Heebo',sans-serif",paddingBottom:80}}>
      ...
      {/* מודל עריכת אורח */}
      {editModal&&<div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setEditModal(null)}>
        <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:20,padding:24,width:"100%",maxWidth:360,direction:"rtl"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
            <div style={{fontSize:16,fontWeight:900}}>{editModal.name}</div>
            <button onClick={()=>setEditModal(null)} style={{width:30,height:30,borderRadius:"50%",border:"1px solid #eee",background:"#f5f5f5",cursor:"pointer",fontSize:16}}>×</button>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
            <div style={{background:"#f9f9f9",borderRadius:10,padding:"10px 14px"}}><div style={{fontSize:10,color:"#888"}}>טלפון</div><div style={{fontSize:13,fontWeight:700}}>{editModal.phone||"לא הוזן"}</div></div>
            <div style={{background:"#f9f9f9",borderRadius:10,padding:"10px 14px"}}><div style={{fontSize:10,color:"#888"}}>כמות מגיעים</div><div style={{fontSize:13,fontWeight:700}}>{editModal.guest_count||1}</div></div>
            <div style={{background:"#f9f9f9",borderRadius:10,padding:"10px 14px"}}><div style={{fontSize:10,color:"#888"}}>קטגוריה</div><div style={{fontSize:13,fontWeight:700}}>{editModal.relation||"לא הוגדר"}</div></div>
            <div style={{background:"#f9f9f9",borderRadius:10,padding:"10px 14px"}}><div style={{fontSize:10,color:"#888"}}>סטטוס</div><div style={{fontSize:13,fontWeight:700,color:rsvpColor(editModal.rsvp)}}>{rsvpLabel(editModal.rsvp)}</div></div>
          </div>
          <button onClick={()=>{setStatusModal(editModal);setEditModal(null);}} style={{width:"100%",background:"#3D5475",color:"#fff",border:"none",borderRadius:12,padding:"12px",fontSize:14,fontWeight:800,cursor:"pointer",fontFamily:"inherit",marginBottom:8}}>✏️ שנה סטטוס הגעה</button>
          <button onClick={async()=>{if(!window.confirm("למחוק?"))return;await sb.from("guests").delete().eq("id",editModal.id);setGuests(gs=>gs.filter(x=>x.id!==editModal.id));setTables(ts=>ts.map(t=>({...t,guests:(t.guests||[]).filter(x=>x.id!==editModal.id)})));setEditModal(null);}} style={{width:"100%",background:"#FFF5F5",color:"#C53030",border:"1px solid #FED7D7",borderRadius:12,padding:"12px",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>🗑️ מחק אורח</button>
        </div>
      </div>}
