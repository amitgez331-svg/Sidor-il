        ...
        <div style={{maxWidth:1080,margin:"0 auto"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:20,maxWidth:1040,margin:"0 auto 40px"}}>
            {[
              ...
              {
                id:"sms",name:"הודעות SMS",price:"₪70",period:"החל מ-150 הודעות",color:"#059669",bg:"#F0FFF6",
                badge:"📱 SMS",
                features:["הכל בחבילת בסיס","שליחת SMS לכל האורחים","תזמון הודעות אוטומטי","אישורי הגעה ב-SMS","תזכורות לפני האירוע"],
              },
              {
                id:"whatsapp",name:"הודעות WhatsApp",price:"₪80",period:"החל מ-50 רשומות",color:"#25D366",bg:"#F0FFF6",
              ...
              {
                id:"vip",name:"VIP הכל כלול",price:"₪250",period:"החל מ-100 רשומות",color:"#B45309",bg:"#FFFBEB",
                badge:"👑 VIP",
...
function PackagesScreen({ event, onBack }) {
  ...
  const packages=[
    ...
    {id:"sms",name:"הודעות SMS",price:70,color:"#059669",icon:"📱",badge:"SMS",
     features:["הכל בחבילת בסיס","שליחת SMS לכל האורחים","תזמון הודעות אוטומטי","תזכורות לפני האירוע","אישורי הגעה ב-SMS"]},
    {id:"whatsapp",name:"WhatsApp אוטומטי",price:80,color:"#25D366",icon:"💬",badge:"WhatsApp",featured:true,
     features:["הכל בחבילת SMS","שליחה אוטומטית בוואטסאפ","תזכורת לפני האירוע","הודעת תודה לאחר האירוע","מגיב אוטומטי לשאלות"]},
    {id:"vip",name:"VIP הכל כלול",price:250,color:"#B45309",icon:"👑",badge:"VIP",
     features:["הכל בחבילת WhatsApp","📞 סבב שיחות טלפוני לאורחים","מספר שולחן ב-SMS ביום האירוע","סגנון הזמנה פרימיום","תמיכה ייעודית 24/7"]},
    {id:"reception",name:"עמדת קבלת פנים",price:1300,color:"#7C3AED",icon:"💎",badge:"פרימיום",
     features:["עמדת קבלת פנים עם דיילים/ות","חלוקת פתקיות הושבה לאורחים","צמצום רזרבות במהלך ההושבה","ניהול תורים בכניסה","תיאום מלא עם צוות האולם"]},
  ];
          ...
          <div key={pkg.id} onClick={()=>setSelectedPkg(selectedPkg?.id===pkg.id?null:pkg)}
            ...
            {selectedPkg?.id===pkg.id&&(
              <div>
                ...
                {/* אם VIP — הצג טבלת מחירים לפי רשומות */}
                {pkg.id==="vip"&&(
                  <div style={{background:C.bg,borderRadius:12,padding:12,marginBottom:12}}>
                    <div style={{fontSize:12,fontWeight:700,color:C.text,marginBottom:8}}>מחיר לפי כמות רשומות:</div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,maxHeight:200,overflowY:"auto"}}>
                      {vipPacks.map(([label,price])=>(
                        <div key={label} style={{border:`1px solid ${C.border}`,borderRadius:8,padding:"7px 8px",display:"flex",justifyContent:"space-between",alignItems:"center",background:C.surface}}>
